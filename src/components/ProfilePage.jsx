    ...
    <main className={`min-h-screen ${pageShellClass}`}>
      <header className={`sticky top-0 z-30 border-b backdrop-blur-xl ${isGitHubPro ? 'border-violet-400/10 bg-[#09051a]/75' : 'border-white/5 bg-[#020617]/80'}`}>
        <div className="mx-auto flex max-w-6xl flex-col gap-3 px-4 py-3 sm:flex-row sm:items-center sm:justify-between sm:px-6">
          <button
            onClick={onBack}
            className="inline-flex w-full items-center justify-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-2 text-xs font-bold text-slate-300 transition-colors hover:bg-white/10 sm:w-auto"
          >
            <ChevronLeft className="h-4 w-4" />
            Back
          </button>
          <div className={`flex items-center gap-2 text-xs font-bold uppercase tracking-[0.25em] sm:tracking-[0.3em] ${isGitHubPro ? 'text-violet-200/70' : 'text-slate-500'}`}>
      ...
      <div className="mx-auto max-w-6xl px-3 py-4 sm:px-4 sm:py-6 lg:px-6 lg:py-8">
        <section className={`overflow-hidden rounded-[2rem] border ${cardShellClass}`}>
          <div className={`border-b px-4 py-4 sm:px-5 ${isGitHubPro ? 'border-violet-400/10 bg-gradient-to-r from-violet-500/18 via-fuchsia-500/8 to-transparent' : 'border-white/5 bg-gradient-to-r from-emerald-500/10 via-cyan-500/5 to-transparent'}`}>
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex items-center gap-3 sm:gap-3.5">
                <img src={profileAvatar} alt="" className={`h-12 w-12 rounded-2xl object-cover ring-2 sm:h-14 sm:w-14 ${isGitHubPro ? 'ring-violet-300/55 shadow-[0_0_24px_rgba(168,85,247,0.18)]' : 'ring-emerald-500/30'}`} />
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    <h1 className={`text-lg font-black leading-tight sm:text-xl ${isGitHubPro ? 'text-violet-100' : 'text-white'}`}>{profileName}</h1>
                  ...
                  <p className={`mt-1 text-xs leading-relaxed sm:text-sm ${isGitHubPro ? 'text-violet-200/70' : 'text-slate-400'}`}>@{username} · {data?.pronouns || 'he/him'}</p>
              ...
              <a href={data?.profileUrl || `https://github.com/${username}`} target="_blank" rel="noreferrer" className={`inline-flex w-full items-center justify-center gap-2 self-start rounded-full border px-3.5 py-2 text-[11px] font-bold transition-colors sm:w-auto sm:py-1.5 ${isGitHubPro ? 'border-violet-300/25 bg-violet-500/12 text-violet-50 hover:bg-violet-500/18' : 'border-white/10 bg-white/5 text-slate-200 hover:bg-white/10'}`}>
          ...
          <div className="grid gap-3 p-3 sm:p-4 sm:p-5 lg:grid-cols-2">
